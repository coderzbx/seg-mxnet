...
inline void ImageSegRecordIOParser<DType>::Init(
  ...
  n_parsed_ = 0;
  overflow = false;
  if (record_param_.seed < 0) {
    rnd_.seed(kRandMagic + rand() % 100000);
  } else {
...
#pragma omp parallel
  {
    ...
    #pragma omp master
      LOG(INFO) << "omp_get_num_procs: " << omp_get_num_procs();
...
inline bool ImageSegRecordIOParser<DType>::ParseNext(DataBatch *out) {
  ...
  if (out->data.size() == 0) {
    ...
    unit_size_[1] = input_label_shape.Size();
  }
