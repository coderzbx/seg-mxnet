      ...
      if (param_.normalization == softmaxfocalout_enum::kBatch) {
        ...
        for (index_t i = 0; i < label_workspace.size(0); ++i) {
          for (index_t j = 0; j < label_workspace.size(1); ++j) {
            ...
            DType scale = pow((1 - out[i][l][j]), param_.gamma) * alphas_[l];
            if (l >= 0 && l < static_cast<int>(out_workspace.size(1))) {
              norm_valid_cnt += scale;
              continue;
            } else {
              // An error here
            }
        ...
        valid_cnt = norm_valid_cnt < 1.0 ? 1 : static_cast<int>(norm_valid_cnt);
      } else {
        valid_cnt = 1;
      }
